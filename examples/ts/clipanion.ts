import * as t from "https://unpkg.com/typanion@3.12.1/lib/index.js";
...
import {
  ...
  Builtins,
  BaseContext,
} from "https://unpkg.com/clipanion@3.2.0-rc.9/lib/advanced/index.mjs";
...
class YarnInstall extends Command<Context> {
  ...
  maxRetries = Option.String(`--max-retries`, `0`, {
  ...
  async execute() {
    this.context.stdout.write(
      `Running an install: ${this.context.cwd}, with ${this.maxRetries} max retries\n`
    );
  }
}
...
class YarnRunExec extends Command<Context> {
  ...
  async execute() {
    this.context.stdout.write(
      `Executing a script named ${this.scriptName} ${this.rest}\n`
    );
  }
}
...
export default class YarnAdd extends Command<Context> {
  dev = Option.Boolean(`-D,--dev`, false, { description: `Use dev mode` });
  peer = Option.Boolean(`-P,--peer`, false, { description: `Use peer mode` });

  exact = Option.Boolean(`-E,--exact`, false, {
    description: `Don't add ^ nor ~`,
  });
  tilde = Option.Boolean(`-T,--tilde`, false, { description: `Use ~` });
  caret = Option.Boolean(`-C,--caret`, false, { description: `Use ^` });

  pkgs = Option.Rest({ required: 1 });
  ...
  static usage = Command.Usage({
    ...
    `,
    examples: [
      [`Add the latest version of a package`, `$0 add lodash`],
      [`Add a specific version of a package`, `$0 add lodash@3.0.0`],
    ],
  });
